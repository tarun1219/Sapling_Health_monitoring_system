...
sender_address = 'tarun12191998@gmail.com'
...
while(1):
    ...
    blue_lower=np.array([18,100,100],np.uint8)
    ...
    for pic, contour in enumerate(contours):
        ...
        if(area>3000):
            (x, y, w, h)=cv2.boundingRect(contour)                                        #motion detection
            img=cv2.rectangle(img, (x, y), (x+w, y+h), (20,100,100), 2)
            cv2.putText(img,"  ",(x,y),cv2.FONT_HERSHEY_SIMPLEX, 0.7, (20,100,100))
    ...
    if cv2.waitKey(4) & 0xFF == ord('c'):
    
        img_name='Captured_Image_'+str(numm)
        a=img_name+".jpeg"
        cv2.imwrite(a,img)
        numm=numm+1
